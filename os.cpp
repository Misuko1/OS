...
CurPath curpath;
int current_uof = -1;   // 当前操作文件在 UOF 中的索引
...
int TypeComd(int k)		//type命令处理函数(显示文件内容)，加上统配符*
{
	// 显示文件内容：type <文件名>，显示指定文件的内容。
	// 若指定文件不存在，则给出错误信息。
	if (k == 1 && strcmp(comd[1], "*") == 0) {  // type *，显示目录所有文件的内容
		short s = curpath.fblock;               //当前目录首块号
		FCB* fcbp;
		char fullpath[PATH_LEN];
		strcpy(fullpath, curpath.cpath);
		if (fullpath[strlen(fullpath) - 1] != '/') strcat(fullpath, "/");

		int count = 0;
		while (s > 0) {                     // 遍历当前目录所有盘块	
			fcbp = (FCB*)Disk[s]; 
			for (int i = 0; i < 4; i++, fcbp++)       // 每个盘块有 4 个目录项
			{
				if (fcbp->FileName[0] == '\0' || fcbp->FileName[0] == (char)0xe5)     // 空项或已删除项跳过
					continue;               
				if (fcbp->Fattrib >= 16)  // 跳过子目录
					continue;

				cout << "\n---------- " << fullpath << fcbp->FileName << " ----------\n";
				count++;
				...
				char* buf = new char[fcbp->Fsize + 2];       // 读取文件内容到缓冲区并显示
				file_to_buffer(fcbp, buf);                  // 复用已有函数读取文件
				cout << buf << endl;
				delete[] buf;
			}
			s = FAT[s];                                // 下一个盘块
	...
	if (k == 0)           // 无参数，显示当前操作文件
	{
		if (current_uof == -1 || uof[current_uof].state == 0)
		{
			cout << "\n没有当前操作文件可显示。\n";
			return -1;
		}
		strcpy(gFileName, uof[current_uof].fname);
		fcbp = uof[current_uof].fp;
		s = uof[current_uof].faddr;
		size = uof[current_uof].fsize;
	}
	else if (k == 1)
	{
	...
	else
	{
		...
		file_to_buffer(fcbp, Buffer);  // 复用已有函数读取内容
		cout << Buffer << endl;
		delete[] Buffer;
	}
	return 1;
...
{
	if (k == 1 && strcmp(comd[1], "*") == 0)  // block *
	{
		short s = curpath.fblock;
		FCB* fcbp;
		char fullpath[PATH_LEN];
		strcpy(fullpath, curpath.cpath);
		if (fullpath[strlen(fullpath) - 1] != '/') strcat(fullpath, "/");
	...
	if (k == 0) //无参数时操作当前文件
	{
		...
		strcpy(temppath, uof[current_uof].fname);
		fcbp = uof[current_uof].fp;
		s = uof[current_uof].faddr;
	}
	else if (k == 1)
	{
		s=FindPath(comd[1], attrib, 1, fcbp);
		if (s < 0) {
			cout << "\n路径名错误!" << endl;
			return -2;
		}
...
int CreateComd(int k)		//create命令处理函数：建立新文件 
{
	...
	current_uof = i_uof;                 // 新增：记录为当前操作文件
	cout << "\n文件" << gFileName << "建立成功\n";
	return 1;							//文件创建成功，返回
}
...
int CloseComd(int k)				//close命令的处理函数：关闭文件 
{
	...
	if (i_uof == current_uof)    // 如果关闭的是当前操作文件，则清除当前操作文件标记
		current_uof = -1;
...
{
	// 在 udtab 中存储被删除文件的块号（保存回收链）
	short i = 0, b0, b, bs;

	if (bb == 0)    // 被删除文件是空文件
		return bb;

	bs = getblock();             // 分配第一个块（链表开头）
	b0 = bs;                     // 初始化 b0，防止未初始化错误
	...
	while (bb > 0)
	{
		...
		if (i == SIZE / 2)       // 当前块写满
		{
			...
			pb = (short*)Disk[b]; // 切换盘块指针
		}
	}

	pb[i] = -1;                   // 链表结束标志
	return bs;                    // 返回链表头（第一个块）
}
...
bool is_wildcard_all(const char* name)     //判断是否是通配符 "*" 的辅助函数
{
...
int DelComd(int k)			//del(删除文件)命令处理函数
{
	...
	if (k == 1 && is_wildcard_all(comd[1]))
	{
		short s = curpath.fblock;// del * 命令 ,删除当前目录下所有文件，不含子目录
		FCB* fcbp;
		int count = 0;
		char yn;

		cout << "\n你确定要删除当前目录下所有文件吗？(y/n) ";
		cin >> yn;
		...
		while (s > 0)
		{
			...
			for (int i = 0; i < 4; i++, fcbp++)
			{
				if (fcbp->FileName[0] == '\0' || fcbp->FileName[0] == (char)0xe5)
					continue;
				if (fcbp->Fattrib >= 16) // 是子目录，跳过
					continue;

				// 检查是否已打开
				strcpy(temppath, curpath.cpath);
				if (temppath[strlen(temppath) - 1] != '/') strcat(temppath, "/");
				strcat(temppath, fcbp->FileName);

				int iu = Check_UOF(temppath);
				if (iu < S)
				{
					cout << "\n文件 " << fcbp->FileName << " 正在使用，跳过删除。\n";
					continue;
				}

				// 执行删除（复用原有逻辑）
				int ret = PutUdtab(fcbp);
				if (ret < 0) {
					cout << "\n磁盘空间不足，无法保存恢复信息，文件 " << fcbp->FileName << " 未删除。\n";
					continue;
				}
				fcbp->FileName[0] = (char)0xe5;
				releaseblock(fcbp->Addr);
				count++;
			}
			s = FAT[s];
		}
...
int CopyComd(int k)		//copy����Ĵ��������������ļ� 
{
	...
	if (k == 2 && is_wildcard_all(comd[1]))
	{
		...
		short src_dir_block = curpath.fblock;
		FCB* src_fcbp;
		int copied = 0;

		while (src_dir_block > 0)
		{
			src_fcbp = (FCB*)Disk[src_dir_block];
			for (int i = 0; i < 4; i++, src_fcbp++)
			{
				...
				*new_fcbp = *src_fcbp;  // ��������FCB
				...
				while (old_head > 0)
				{
					...
					memcpy(Disk[curr], Disk[old_head], SIZE);
					prev = curr;
					old_head = FAT[old_head];
				}
				FAT[prev] = -1;

				copied++;
			}
			src_dir_block = FAT[src_dir_block];
		}
...
int RewindComd(int k)	//rewind命令的处理函数：读、写指针移到文件开头 
{
	...
	if (uof[i_uof].faddr == 0) {
		uof[i_uof].readp = 0;
		uof[i_uof].writep = 1;          // 约定空文件写指针指向第1个位置
	}
	else {
