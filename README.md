...
做了如下工作：
1. 同名文件覆盖支持（4.9）
改善目标： 改变了原始程序中遇到同名文件就报错终止的行为，改为询问用户是否覆盖，增强了命令的灵活性。


代码实现： 在处理非通配符复制时，如果目标目录中已存在同名文件，代码执行以下逻辑 ：


判断文件类型： 如果同名项是子目录，则禁止覆盖并报错（不能覆盖子目录）。


询问用户： 如果同名项是文件，则提示 存在文件与目标文件同名，是否要覆盖它？(y/n) 。

执行覆盖： 如果用户选择 'y' 或 'Y'，程序会执行以下关键步骤：

回收原目标文件占用的所有盘块（将 FAT 表项清零）。

重置原目标文件 FCB 的首块号和大小 。

将源文件内容复制到这些已清空盘块的地址上，并复用原目标文件的 FCB，实现数据覆盖 。

2. 完善copy命令，改正某些缺陷（4.10）
目标： 解决 copy boy / 和 copy boy .. 等命令的路径解析错误。
